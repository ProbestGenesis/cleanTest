...
export const verifyPurchaseValidationCode = async (
  ...
  try {
    ...
    if (!row.purchase) return { ok: false as const, message: 'Achat lié introuvable' }
    if (row.purchase.status !== 'DELIVERED') {
      return { ok: false as const, message: "Cet achat n'est pas encore prêt pour la confirmation" }
    }
