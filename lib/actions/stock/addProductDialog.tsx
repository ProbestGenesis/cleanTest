...
import { ImageIcon, ImagePlus, Plus, Trash2, UploadCloud, X } from 'lucide-react'
...
function ProductDialog({ isOpen, onClose, type, data, mutate }: Props) {
  ...
  const onSubmit = async (value: z.infer<typeof formSchema>) => {
    ...
    const { ok, message: resMessage } = await addProduct({ ...value, images: finalImages })
    setMessage({ ok, text: resMessage })
    if (ok) {
      if (mutate) {
        await mutate()
      } else {
